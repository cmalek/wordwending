...
"""Hosted Hugging Face olmOCR batch execution without local inference."""
...
#: Declared olmOCR runner input and batching contract for hosted execution.
...
class HuggingFaceOlmocrRunner:
    """
    Execute planned batches against one hosted OpenAI-compatible olmOCR endpoint.
    ...
    def __init__(
        ...
        """
        Bind one hosted olmOCR runner to endpoint settings and an HTTP client.
    ...
    def invoke(
        ...
        """
        Execute one packaged batch against the hosted olmOCR endpoint.
    ...
    def _completion_payload(self, image_base64: str) -> dict[str, Any]:
        """
        Build one OpenAI-compatible olmOCR completion request body.
